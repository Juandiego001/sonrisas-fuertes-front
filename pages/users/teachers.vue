...
v-container(fluid)
  ...
  :options.sync="options" :search="search")
    template(#item.options="{ item }")
      v-btn.mr-2(color="success" depressed icon @click="getTeacher(item)")
        v-icon mdi-pencil
    ...
    v-form(ref="form" @submit.prevent="saveTeacher")
      v-card(flat :tile="$vuetify.breakpoint.smAndDown")
        v-card-title.primary.white--text
          | {{ form._id ? 'Formulario editar profesor' : 'Formulario crear profesor' }}
          v-spacer
          v-btn.white--text(icon @click="dialogEdit=false")
            v-icon mdi-close
        v-card-text.my-3
          v-row(dense)
            v-col.primary--text(cols="12" md="12") Información del profesor
            v-col(cols="12" md="6")
              text-field(
                v-model="form.name"
                label="Nombre"
