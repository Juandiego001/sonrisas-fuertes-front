...
<script>
import { publicationUrl, fileUrl, linkUrl } from '~/mixins/routes'
      ...
      try {
        ...
        if (this.form._id) {
          ({ message } = await this.$axios.$patch(
              `${publicationUrl}${this.form._id}`, this.getFormData()))
        } else {
          ({ message } = await this.$axios.$post(
            publicationUrl, this.getFormData()))
        }
        this.getData()
